...
func LoadClientTLSConfig(
	...
	providerType, namespace string,
) (*TLSConfig, error) {
	// Construct secret name: external-secrets-provider-{type}-tls
	secretName := fmt.Sprintf("external-secrets-provider-%s-tls", strings.ToLower(providerType))
	...
	// Construct server name (DNS name of provider service)
	serverName := fmt.Sprintf("provider-%s.%s.svc", strings.ToLower(providerType), namespace)
