...
load_images_to_kind() {
    ...
    # Provider image
    local provider_image="ghcr.io/external-secrets/provider-kubernetes:latest"
    if docker image inspect "$provider_image" &> /dev/null; then
        log_info "Loading provider image: $provider_image"
        kind load docker-image "$provider_image" --name "$cluster_name"
    else
        log_warning "Provider image not found locally: $provider_image"
        log_warning "Attempting to pull from registry (may fail if not published)"
    fi
...
verify_installation() {
    ...
    if ! kubectl wait --for=condition=ready pod \
        ...
        -n "$NAMESPACE" \
        --timeout=300s; then
        log_error "Provider pod not ready"
        kubectl get pods -n "$NAMESPACE"
