...
// CreateKubernetesProvider creates a Kubernetes provider CRD with standard configuration.
// Uses default service account auth and returns the created provider object.
func CreateKubernetesProvider(f *framework.Framework, namespace, name, remoteNamespace string, caBundle []byte) *k8sv2alpha1.Kubernetes {
	k8ss := &k8sv2alpha1.Kubernetes{
		...
		Spec: k8sv2alpha1.KubernetesSecretSpec{
			Server: &k8sv2alpha1.KubernetesServer{
				URL:      "https://kubernetes.default.svc",
				CABundle: caBundle,
			},
			RemoteNamespace: remoteNamespace,
			Auth: &k8sv2alpha1.KubernetesAuth{
				ServiceAccount: &k8sv2alpha1.KubernetesServiceAccount{
					Name:      "default",
					Namespace: namespace,
				},
			},
...
// CreateProviderConnection creates a ProviderConnection pointing to the specified provider.
// Uses standard provider-kubernetes service address and returns the created ProviderConnection object.
func CreateProviderConnection(f *framework.Framework, namespace, name, providerName, providerNamespace string) *v1.Provider {
