...
var _ = Describe("V2 End-to-End Tests", Label("v2"), func() {
	...
	Describe("Kubernetes Provider", func() {
		...
		It("should sync secrets across namespaces", func() {
			...
			CreateKubernetesProvider(f, targetNamespace.Name, "k8s-store", sourceNamespace.Name, caBundle)
			CreateProviderConnection(f, targetNamespace.Name, secretStoreName, "k8s-store", targetNamespace.Name)
			WaitForProviderConnectionReady(f, targetNamespace.Name, secretStoreName, 30*time.Second)
					...
					Data: []esv1.ExternalSecretData{
						{
							...
							RemoteRef: esv1.ExternalSecretDataRemoteRef{
								Key:      fmt.Sprintf("%s/%s", sourceNamespace.Name, sourceSecretName),
								Property: "username",
							},
						},
						{
