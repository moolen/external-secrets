...
var _ = Describe("[v2] GetAllSecrets", Label("v2", "get-all-secrets"), func() {
	...
	It("should find secrets by tags (labels)", func() {
		caBundle := GetClusterCABundle(f, targetNamespace.Name)
		CreateKubernetesProvider(f, targetNamespace.Name, "k8s-provider", sourceNamespace.Name, caBundle)
		CreateProviderConnection(f, targetNamespace.Name, "test-secretstore", "k8s-provider", targetNamespace.Name)
		WaitForProviderConnectionReady(f, targetNamespace.Name, "test-secretstore", 30*time.Second)
		...
		Eventually(func() bool {
			...
			return false
		}, 60*time.Second, 2*time.Second).Should(BeTrue(), "ExternalSecret should become ready")
	...
	It("should find secrets by name regexp", func() {
		caBundle := GetClusterCABundle(f, targetNamespace.Name)
		CreateKubernetesProvider(f, targetNamespace.Name, "k8s-provider-regex", sourceNamespace.Name, caBundle)
		CreateProviderConnection(f, targetNamespace.Name, "test-secretstore-regex", "k8s-provider-regex", targetNamespace.Name)
		WaitForProviderConnectionReady(f, targetNamespace.Name, "test-secretstore-regex", 30*time.Second)
