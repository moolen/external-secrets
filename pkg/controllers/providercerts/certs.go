...
func (r *ProviderCertReconciler) getProviderDNSNames(config *ProviderCertConfig) []string {
	...
	for _, serviceName := range config.ServiceNames {
		...
		dnsNames = append(dnsNames, fmt.Sprintf("%s.%s", serviceName, config.Namespace))
		dnsNames = append(dnsNames, fmt.Sprintf("%s.%s.svc", serviceName, config.Namespace))
		dnsNames = append(dnsNames, fmt.Sprintf("%s.%s.svc.cluster.local", serviceName, config.Namespace))
	}
	return dnsNames
}
