...
func (r *Reconciler) pushSecretToProviderV2(ctx context.Context, storeRef esv1alpha1.PushSecretStoreRef, ps esv1alpha1.PushSecret, secret *corev1.Secret) (map[string]esv1alpha1.PushSecretData, error) {
	...
	for _, data := range ps.Spec.Data {
		...
		case esv1alpha1.PushSecretUpdatePolicyIfNotExists:
			...
			exists, err := grpcClient.SecretExists(ctx, remoteRef, providerConfigJSON)
			if err != nil {
		...
		// Push the secret
		if err := grpcClient.PushSecret(ctx, secretData, pushData, providerConfigJSON); err != nil {
...
// fetchProviderConfig fetches the provider-specific configuration CRD.
func (r *Reconciler) fetchProviderConfig(ctx context.Context, ref *esapi.ProviderReference) (interface{}, error) {
	// For now, we support a simple set of provider types
	// In production, this could be more dynamic
	switch ref.Kind {
	case "Kubernetes":
		var k8sProvider k8sv2alpha1.Kubernetes
		nsName := types.NamespacedName{
			Name:      ref.Name,
			Namespace: ref.Namespace,
		}
		if err := r.Client.Get(ctx, nsName, &k8sProvider); err != nil {
			return nil, fmt.Errorf("failed to get Kubernetes provider: %w", err)
		}
		return &k8sProvider, nil
	case "AWSSecretsManager":
		// Use unstructured for AWS provider
		u := &unstructured.Unstructured{}
		u.SetAPIVersion("provider.external-secrets.io/v2alpha1")
		u.SetKind("AWSSecretsManager")
		nsName := types.NamespacedName{
			Name:      ref.Name,
			Namespace: ref.Namespace,
		}
		if err := r.Client.Get(ctx, nsName, u); err != nil {
...
// serializeProviderConfig serializes the provider config to JSON.
// The controller sends only the Spec portion to the provider.
func (r *Reconciler) serializeProviderConfig(providerConfig interface{}) ([]byte, error) {
	// Extract the Spec from the provider config
	// For now, we'll marshal the whole thing and let the provider extract the Spec
	configJSON, err := json.Marshal(providerConfig)
	if err != nil {
		return nil, fmt.Errorf("failed to marshal provider config: %w", err)
	}

	// Parse and extract just the Spec field
	var wrapper struct {
	...
	if len(wrapper.Spec) == 0 {
		return nil, fmt.Errorf("provider config spec is empty")
	}
...
func (r *Reconciler) DeleteSecretFromProvidersV2(ctx context.Context, ps *esv1alpha1.PushSecret, newMap esv1alpha1.SyncedPushSecretsMap, stores map[esv1alpha1.PushSecretStoreRef]interface{}) (esv1alpha1.SyncedPushSecretsMap, error) {
	...
	for storeName, oldData := range ps.Status.SyncedPushSecrets {
		...
		if v2Store, ok := matchingStore.(*esapi.Provider); ok {
			// Get the v2 Provider and create gRPC client
			providerConfig, err := r.fetchProviderConfig(ctx, &v2Store.Spec.Config.ProviderRef)
			if err != nil {
				return out, fmt.Errorf("failed to fetch provider config: %w", err)
			}

			providerConfigJSON, err := r.serializeProviderConfig(providerConfig)
			if err != nil {
				return out, fmt.Errorf("failed to serialize provider config: %w", err)
			}

			tlsConfig, err := grpc.LoadClientTLSConfig(ctx, r.Client, v2Store.Spec.Config.ProviderRef.Kind, "external-secrets-system")
			if err != nil {
			...
			if !ok {
				...
				for _, oldRef := range oldData {
					...
					if err := grpcClient.DeleteSecret(ctx, remoteRef, providerConfigJSON); err != nil {
